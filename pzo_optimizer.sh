...
echo "================================================================="
echo " Project Zomboid Build 42 Engine Optimizer (v0.7.0)"
...
if [ "$OS_TYPE" = "Darwin" ]; then
    ...
    if [ "$ZB_FOUND" -eq 1 ]; then
        ...
        echo "PZO v0.7.0+ automatically runs your ZombieBuddy Workshop mods natively!"
        echo ""
...
# ==============================================================================
    ...
    if [ -f "$PZ_DIR/ZombieBuddy.jar" ] || [ -f "$PZ_DIR/zbNative.so" ] || [ -f "$PZ_DIR/zbNative.dylib" ] || [ -f "$PZ_DIR/zombiebuddy.json" ]; then
        ...
        echo "PZO v0.7.0+ automatically runs your ZombieBuddy mods natively!"
        echo ""
        echo "[+] ZombieBuddy detected! Coexistence mode enabled." 
    fi
